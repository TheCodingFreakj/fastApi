...
def _new_session(shop_url):
    ...
    API_KEY = apps.get_app_config('shopify_auth').SHOPIFY_API_KEY
...
def login(request):
    ...
    return render(request, '/app/build/app/build/index.html', {})
...
def authenticate(request):
    ...
    scope = apps.get_app_config('shopify_auth').SHOPIFY_API_SCOPE.split(",")
    print(scope)
...
def finalize(request):
    ...
    return redirect(request.session.get('return_to', reverse('home')))
